...
Item {
  ...
  function refresh() {
    ...
    observeCall = runtime.invoke("device.observe", "observe", {
      fields: ["identity", "connection", "battery", "supported-controls", "listening-mode", "adaptive-level", "conversation-awareness", "one-bud-anc", "ear-detection", "case-lid"]
    })
  ...
  function _send(verb, field, optimistic) {
    ...
    var call = runtime.invoke("device.control", "control", {
      control: operation.name, value: operation.value
    })
    if (!call) {
      _clearPending(field)
      return
